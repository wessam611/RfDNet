...
class SkipPropagation(nn.Module):
    ...
    def __init__(self, cfg, optim_spec=None):
        ...
        self.input_feature_dim = int(
            cfg.config['data']['use_color_completion']) * 3 + int(not cfg.config['data']['no_height']) * 1
        ...
        self.point_seg = PointSeg(
            num_class=2, channel=self.input_feature_dim + 3)
    ...
    def generate(self, box_xyz, box_orientations, box_feature, input_point_cloud):
        ...
        # labels are not used in generation.
        point_instance_labels = torch.zeros_like(features)
        ...
        input_features = input_features.permute([0, 2, 3, 1]).contiguous().view(
            batch_size * N_proposals, N_points, -1)

        # use PointNet to predict masks
        seg_pred, trans_feat = self.point_seg(
            input_features.transpose(1, 2).contiguous())
        seg_pred = seg_pred.contiguous().view(batch_size * N_proposals * N_points, 2)

        box_feature = box_feature.transpose(1, 2).contiguous().view(
            batch_size * N_proposals, -1).unsqueeze(1)
        box_feature = box_feature.repeat(1, N_points, 1)
        input_features = torch.cat([input_features, box_feature], dim=2)

        # get segmented masks
        point_seg_mask = torch.argmax(seg_pred, dim=1).view(
            batch_size*N_proposals, N_points)
        point_seg_mask = point_seg_mask.unsqueeze(-1).expand(
    ...
    def forward(self, box_xyz, box_orientations, box_feature, input_point_cloud, point_instance_labels, proposal_instance_labels, input_point_normals=None):
        '''
        ...
        features = torch.cat(
            [features, point_instance_labels.unsqueeze(1)], dim=1)

        if input_point_normals is not None:
            xyz, features, normals = self.stn(
                xyz, features, box_xyz, box_orientations, input_point_normals)
        else:
            xyz, features = self.stn(xyz, features, box_xyz, box_orientations)
        ...
        instance_point_masks = instance_labels == proposal_instance_labels.unsqueeze(
            -1).repeat(1, 1, N_points)
        instance_point_masks = instance_point_masks.view(
            batch_size * N_proposals * N_points)
        ...
        seg_pred, trans_feat = self.point_seg(
            input_features.transpose(1, 2).contiguous())
        seg_pred = seg_pred.contiguous().view(batch_size * N_proposals * N_points, 2)
        point_mask_loss = self.mask_loss_func(
            seg_pred, instance_point_masks.long(), trans_feat, weight=None)
